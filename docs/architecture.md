...
This remains bounded source, deterministic-fixture, and automated CI evidence, not a completed provider or commerce release unit. A checked-in Draft 2020-12 runner compiles the registered schemas and resolves their references; extension invariants remain mapped to runtime tests. Attributable CI passes PHPUnit coverage generation, PHP 8.1–8.4 deterministic suites, an exact three-cell packaged WordPress/WooCommerce/database smoke, and an isolated Chromium/axe fixture. Deterministic whole-workflow product grounding, production requirement semantic promotion, broad HPOS/Action Scheduler/provider/platform evidence, and formal acceptance remain incomplete. Catalog governance continues to block the seven `implemented_not_tested` handlers.
...
Proposed implementation minima are WordPress 6.5, PHP 8.1, and WooCommerce 8.5 with no arbitrary upper bound. These are engineering gates, not broad tested-compatibility claims. The exact CI smoke passes WP 6.5/WC 8.5.2/PHP 8.1/MySQL 8.0 and WP 7.1/WC 11.0.1/PHP 8.4 on MySQL 8.0 and MariaDB 11.4. It exercises bounded public product/order CRUD under legacy and HPOS storage and separately asserts classic and Checkout Blocks page fixtures; it does not execute complete checkout flows. This does not promote the compatibility matrix, which remains `Not assessed`. Boot, feature exposure, protected-media composition, and provider transmission require the stored schema to equal `VEYRA_SCHEMA_VERSION`; unknown newer schemas do not run older code. WooCommerce order access is through CRUD for HPOS safety. Action Scheduler APIs are called only after WordPress `init` priority 1 has run or from `action_scheduler_init`, and jobs remain application-idempotent. No broad HPOS or Action Scheduler acceptance evidence exists.
