...
This package combines the canonical Veyra v4.1 design baseline with an installable modular-monolith plugin, runtime contracts, migrations, user interfaces, deterministic tests, and release evidence. Substantial Production Core paths are implemented, but incomplete core tool coverage and unexecuted live integration, privacy, accessibility, security, provider-evaluation, and acceptance gates prevent a production claim.
...
- `compatibility-matrix.csv` — proposed minima only; every row remains untested.
